...
- three primary operations：首页主线统一为添加资料 / 搜索回顾 / 生成 AI 上下文包，对应 organize / review / extract；remind 保留为兼容和计划任务能力，不放回首页主线。
- obsidian workflow：所有沉淀结果优先写成 Obsidian 新笔记，保留来源和下一步。
- human-readable GUI：站点式知识库 + 成熟个人知识工作台风格，默认展示可读结果，诊断细节只在工具维护页按需查看。
...
- lightweight daily triage：每天 9 点只生成最多 3 条行动建议，不做定时整理。
...
- legacy compatibility：旧 action 保留兼容，但只放在 `/advanced` 工具维护页，不放回首页主线。
